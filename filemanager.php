...
<!-- DO NOT EDIT THIS BLOCK OF PHP! IT WILL WIPE OUT YOUR WHOLE WEBSITE IF INCORRECTLY MODIFIED! -->
<?php
function rrmdir($dir)
{
    if (is_dir($dir)) {
    ...
    <table>
        ...
        foreach ($files as $file) {
            if ($file[0] != '.') {
                ...
                echo "<tr ondblclick='fileClicked(&apos;" .  $file . "&apos;, &apos;" . $type . "&apos;)'><td>" . $select . "</td><td>" . $file . "</td><td>" . $type . "</td><td>" . $owner . "</td><td>" . $size . "</td><td>" . $modDate . "</td></tr>";
            }
        }
...
<script>
    function fileClicked(file, type) {
        if (type != "folder") {
            window.open("fileEditor.php?file=" + file + "&type=" + type);
        }
        //alert(file);
